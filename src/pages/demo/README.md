...
* UI 库(必选)
  ...
  > 因为我们是要写页面, 我们的目标是创造出一个可视化界面(GUI), 所以 UI 库可以认为是必须的. 即先通过 UI 库将页面搭起来, 让页面有内容"可视"(HTML+CSS). 其他功能则需要根据不同的场景和要求, 选择合适的方案来实现, 让页面有交互"可用"(JS)
* 工具库(场景)

  > 见招拆招: 备好应对各种场景的工具箱, 针对场景准备好多套解决方案(优缺点选型)
* 业务逻辑(商业)
  ...
  >
  > 概括起来, 即业务逻辑是要通过页面搭建起一套产品, 通过逻辑控制业务流程, 完成商业价值: 自动化 + 场景化 = 智能化
